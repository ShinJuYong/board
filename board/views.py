...
def signin(request):
    ...
    # 회원정보 조회
        ...
        try:
            ...
            request.session['email'] = email               # session
...
def map_data(request):
    ...
    for d in data:
        ...
        if(dist <= 300): # 300km 이내의 장소만 응답결과로 저장
            map_list.append(d)
...
def contact(request):
    if request.method == 'POST':
        ...
        # 발신자주소, 수신자주소, 메시지
        send_mail('hacker961011@gmail.com', email, comment)
...
def send_mail(from_email, to_email, msg):
    ...
    smtp.login(from_email, 'rlzfwdahbnflvqvm') # 인증정보 설정
    msg = MIMEText(msg)
